...
| MIN_STEP                | 最小步数(每天22点时达到的目标下限)                                                                            |
| MAX_STEP                | 最大步数(每天22点时达到的目标上限)，实际随机范围随时间线性增加：执行时范围为 当前时间/22点 * MIN_STEP ~ MAX_STEP，北京时间22点达到最大值。同一天内多次执行步数只增不减，模拟真实走路累积 |
...
| DINGTALK_WEBHOOK_TOKEN | 钉钉推送通知的token，在钉钉群中添加自定义机器人后，Webhook地址形如：https://oapi.dingtalk.com/robot/send?access_token={机器人的token}，这里配置{机器人的token}，也支持直接粘贴完整的Webhook地址 |
| DINGTALK_WEBHOOK_SECRET | 钉钉机器人安全设置为加签时对应的密钥，以SEC开头。如果机器人安全设置使用的是自定义关键词或IP白名单，则无需配置。注意使用自定义关键词时，请保证关键词包含在推送标题或内容中（标题固定包含"刷步数"字样，推荐关键词直接填写：刷步数） |
...
- 未做任何配置时，脚本每次执行完后会自动重新随机下一次的执行计划：每天在北京时间 `8~22` 点之间随机抽取若干个小时（默认6个）并随机分钟执行，执行时间每天都不固定，更接近真实运动习惯。
- 可以添加名为 `CRON_RANDOM_COUNT` 的Variables变量修改每天随机执行的次数，值为整数，例如 `5` 表示每天随机执行5次。

#### 方式1、添加名为 `CRON_HOURS` 的Variables变量固定执行小时 `Settings-->Secrets and variables-->Actions-->New repository variables` 注意不是Secret
...
- 注意：配置了 `CRON_HOURS` 后将使用固定小时+随机分钟的策略，`CRON_RANDOM_COUNT` 将不再生效
...
- 步骤：
  ...
  - 钉钉配置Secrets：`INSPECT_DINGTALK_WEBHOOK_TOKEN` 配置钉钉群自定义机器人的access_token（或完整Webhook地址），如果机器人开启了加签，还需要配置`INSPECT_DINGTALK_WEBHOOK_SECRET`（SEC开头的密钥）。
  - 然后点击Actions，左侧选择 `提取配置信息` 手动运行它，运行成功后，将配置信息发送到企业微信、钉钉或telegram中。企业微信、钉钉或者telegram的推送自己按需选择，如果都不配置，请使用日志打印的方式。
...
1. 默认每天在北京时间8~22点间随机执行6次（可通过 `CRON_RANDOM_COUNT` 变量调整次数），小时和分钟均为随机值，每次执行后自动重新随机下一次的执行计划。

- 如果配置了 `CRON_HOURS` Variable变量，则小时固定为配置值，分钟随机，且脚本将自动判断，例如8:05分执行后，将从小时中剔除8，即8:00-8:59都不会再重复执行，避免随机的步数混乱。
...
8. 最大步数和最小步数随着时间增长，10点执行时范围为10/22 \* 18000 ~ 10/22 \* 25000：8181 ~
   ...
   同一天内多次执行时步数只增不减：随机下界会取当天已提交的步数，模拟真实走路累积（例如9点提交了3000步，12点执行时随机范围至少从3000起），跨天后自动重新从0累积。
